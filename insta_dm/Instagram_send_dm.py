...
#     last_message = find_css('div.x78zum5', browser)
    
    last_message = find_css('div.x78zum5', browser).text.split('\n')
    
    if last_message[-5] == '보낸 메시지':
        if not image_path:
            pass
        else:
            # Image area
...
def start_dm(num_per_user, insta_id_list, dm_id_list, timesleep, image_path, message_content):
    ...
    while len(dm_id_list) > url_count:
        for info in range(len(insta_id_list[0])):
            ...
            while True:
                a = insta_login(INPUT_ID, INPUT_PW, insta_id, insta_pw, browser)
                print(a)
                if a:
                    break
